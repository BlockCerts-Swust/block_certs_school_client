...
from src.cert_issuer import helpers
from pycoin.serialize import b2h
from src.cert_issuer.models import CertificateHandler, BatchHandler

from src.cert_issuer.signer import FinalizableSigner
...
class CertificateV2Handler(CertificateHandler):
    ...
    def add_proof(self, certificate_metadata, merkle_proof):
        ...
        certificate_metadata.signed_cert = certificate_json

    def _get_certificate_to_issue(self, certificate_metadata):
        # with open(certificate_metadata.unsigned_cert_file_name, 'r', encoding='UTF-8') as unsigned_cert_file:
        #     certificate_json = json.load(unsigned_cert_file)
        return certificate_metadata.unsigned_cert
...
class CertificateBatchHandler(BatchHandler):
    ...
    def post_batch_actions(self, config):
        ...
        for _, metadata in self.certificates_to_issue.items():
            metadata.blockcert = metadata.signed_cert
        return self.certificates_to_issue
    ...
    def _process_directories(self, config):
        unsigned_certs = config["unsigned_certificates"]
