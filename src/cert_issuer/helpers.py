...
from src.cert_issuer.errors import NoCertificatesFoundError
...
class CertificateMetadata(object):
    def __init__(self, uid, unsigned_cert, signed_cert, blockcert, final_blockcert):
        self.uid = uid
        ...
        self.unsigned_cert  = unsigned_cert
        self.signed_cert = signed_cert
        self.blockcert = blockcert
        self.final_blockcert = final_blockcert


def prepare_issuance_batch(unsigned_certs):
    """
    ...
    for uid in sorted(unsigned_certs):
        certificate_metadata = CertificateMetadata(uid=uid,
                                                   unsigned_cert=unsigned_certs[uid],
                                                   signed_cert=None,
                                                   blockcert=None,
                                                   final_blockcert=None)
...
# def copy_signed_certs(certificates_metadata):
#     for _, metadata in certificates_metadata.items():
