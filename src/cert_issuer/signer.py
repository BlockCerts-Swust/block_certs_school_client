...
from src.cert_issuer.models import SecretManager
logging.getLogger().setLevel(logging.INFO)

class FileSecretManager(SecretManager):
    def __init__(self, signer, secret_key, safe_mode=True, issuing_address=None):
    ...
    def start(self):
        if self.safe_mode:
            check_internet_off(self.secret_key)
        else:
            logging.warning(
        ...
        self.wif = self.secret_key

    def stop(self):
        self.wif = None
        if self.safe_mode:
            check_internet_on(self.secret_key)
        else:
            logging.warning(
                'app is configured to skip the wifi check when the USB is plugged in. Read the documentation to'
                ' ensure this is what you want, since this is less secure')
...
def check_internet_off(secret_key):
    """If internet off and USB plugged in, returns true. Else, continues to wait..."""
    while True:
        if internet_on() is False and secret_key:
            break
        else:
...
def check_internet_on(secret_key):
    """If internet on and USB unplugged, returns true. Else, continues to wait..."""
    while True:
        if internet_on() is True and not secret_key:
            break
        else:
